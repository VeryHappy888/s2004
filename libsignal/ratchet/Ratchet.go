...
func CalculateSenderSession(parameters *SenderParameters) (*session.KeyPair, error) {
	...
	secret = kdf.CalculateSharedSecret(publicKey, privateKey)
	...
	log.Println("masterSecret", hex.EncodeToString(masterSecret))
	// Derive the root and chain keys based on the master secret.
	// TODO root.KdfInfo 可能需要修改成 WhisperText 会导致 mac 校验不过过导致消息无法解密
	...
	log.Println("derivedKeysBytes", hex.EncodeToString(derivedKeysBytes))
