...
import (
	...
	"ws-go/libsignal/ratchet"
	"ws-go/libsignal/serialize"
	"ws-go/libsignal/state/record"
	"ws-go/libsignal/state/store"
	"ws-go/libsignal/util/medium"
...
func (b *Builder) Process(message *protocol.PreKeySignalMessage) (unsignedPreKeyID *optional.Uint32, err error) {
	...
	// CheckNil to see if the keys are trusted.
...
func (b *Builder) processV3(sessionRecord *record.Session,
	...
	// CheckNil to see if we've already set up a session for this V3 message.
	...
	if ourSignedPreKeyRecord == nil {
		return nil, errors.New("not message.SignedPreKeyID()")
	}
...
func (b *Builder) ProcessBundle(preKey *prekey.Bundle) error {
	...
	// CheckNil to see if the bundle has a signed pre key.
	...
	//pubKey,_ := base64.StdEncoding.DecodeString("Bc1uI8/IQMoOmnTyHRLglvtG8jjgsNt+2xztM6vYe3dt")
	//priKey, _ := hex.DecodeString("28a328ceaac796b36b071d16c85e631653a302093ebdf35bcb8d8ec93d1dd66f")
	//djbECPub := ecc.NewDjbECPublicKey(bytehelper.SliceToArray(pubKey))
	//djbECPriv := ecc.NewDjbECPrivateKey(bytehelper.SliceToArray(priKey))
	//ourBaseKey = ecc.NewECKeyPair(djbECPub,djbECPriv)
	...
	//test
	//pubKey1,_ := base64.StdEncoding.DecodeString("BUBKTCARsqn1++WRW3iugTXQrfpffWdtR+OF7k4QXLZ6")
	//priKey1, _ := hex.DecodeString("38d916c4163bbc310e43cbc01df01cc86e44a55c85eafe97c4d41a0d70ce6962")
	//djbECPub1 := ecc.NewDjbECPublicKey(bytehelper.SliceToArray(pubKey1))
	//djbECPriv1 := ecc.NewDjbECPrivateKey(bytehelper.SliceToArray(priKey1))
	//sendingRatchetKey = ecc.NewECKeyPair(djbECPub1,djbECPriv1)
