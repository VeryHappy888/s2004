...
// StateSerializer is an iface for serializing and deserializing
// a Signal State into bytes. An implementation of this iface should be
...
// The session state is implemented as a entity rather than protobuffers
...
func (s *State) UnackPreKeyMessageItems() (*UnackPreKeyMessageItems, error) {
	...
	log.Println("baseKey", hex.EncodeToString(baseKey.Serialize()))
	if err != nil {
		return nil, err
	}
...
func (s *State) structure() *StateStructure {
	...
	var preKeyStructure *PendingPreKeyStructure
	if s.pendingPreKey != nil {
		preKeyStructure = s.pendingPreKey.structure()
	}
