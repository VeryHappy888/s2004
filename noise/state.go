...
func (s *CipherState) Decrypt(out, ad, ciphertext []byte) ([]byte, error) {
	...
	//fmt.Println("s.n",s.n)
	//fmt.Println("ad",hex.EncodeToString(ad))
...
func (s *symmetricState) InitializeSymmetric(handshakeName []byte) {
	//fmt.Println(string(handshakeName))
	h := s.cs.Hash()
	...
	copy(s.ck, s.h)
	//fmt.Println(hex.EncodeToString(s.ck),hex.EncodeToString(s.h))
}
...
func (s *symmetricState) MixKey(dhOutput []byte) {
	...
	copy(s.k[:], hk)
	//fmt.Println("s.k",hex.EncodeToString(s.k[:]))
	s.c = s.cs.Cipher(s.k)
	s.n = 0
}
...
func (s *symmetricState) MixHash(data []byte) {
	...
	//fmt.Println(hex.EncodeToString(append(s.h,data...)))
	s.h = h.Sum(s.h[:0])
	//fmt.Println(hex.EncodeToString(s.h))
}
...
func NewHandshakeState(c Config) (*HandshakeState, error) {
	...
	//fmt.Println(string(hs.ss.cs.Name()))
	hs.ss.InitializeSymmetric([]byte("Noise_" + c.Pattern.Name + "_" + string(hs.ss.cs.Name())))
	...
	for _, msg := range s.messagePatterns[s.msgIdx] {
		...
		case MessagePatternE, MessagePatternS:
			...
			case MessagePatternE:
				...
				//fmt.Println("s.re",hex.EncodeToString(s.re))
			case MessagePatternS:
				if len(s.rs) > 0 {
					return nil, nil, nil, errors.New("noise: invalid state, rs is not nil")
				}
				//fmt.Println("message",hex.EncodeToString(message[:expected]))
		...
		case MessagePatternDHEE:
			//fmt.Println("s.re",hex.EncodeToString(s.re))
			//fmt.Println("s.e.priKey",hex.EncodeToString(s.e.Private))
			s.ss.MixKey(s.ss.cs.DH(s.e.Private, s.re))
		case MessagePatternDHES:
			if s.initiator {
				s.ss.MixKey(s.ss.cs.DH(s.e.Private, s.rs))
