...
// NewPromiseTimeOut 创建一个自动超时的 Promise
func NewPromiseTimeOut(duration time.Duration) *Promise {
	/*promise := &Promise{
		pending: true,
		executor: func(resolve func(Any), reject func(error)) {
			timeout, _ := context.WithTimeout(context.Background(), duration)
			<-timeout.Done()
			reject(errors.New("run time our!"))
		},
		result: nil,
		err:    nil,
		mutex:  sync.Mutex{},
...
func (promise *Promise) SuccessResolve(resolution Any) {
